---
title: "Quickstart"
description: "Install the Obi widget, identify a user, run a session, and pull your first report."
---

This guide takes you through the whole loop: add the widget, tell it who the user is, start a session, and read the results.
...
The widget now loads on every page. We recommend hiding it on your login page — see [User management](/sdk/sessions#start-obi-after-login).

## 2. Identify the user

After a user logs in, tell Obi who they are. This drives targeting, personalisation, and reporting.
...
Once a user has run a session, fetch it from the Session Reporting API. This call is server-side and uses your **API token**.
